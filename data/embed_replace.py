...
class EmbedReplace():
    ...
    def extract_keywords(self, dct, tfidf, threshold=0.2, topk=5):
        """find high TFIDF score keywords
        Args:
            dct (Dictionary): gensim.corpora Dictionary  a reference Dictionary
            tfidf (list of tfidf):  model[doc]  [(int, number)]
            threshold (float) : high TFIDF score must be greater than the threshold
            topk(int): num of highest TFIDF score 
        Returns:
...
wv_path = 'word_vectors/sgns.baidubaike.bigram-char'  # https://github.com/Embedding/Chinese-Word-Vectors
