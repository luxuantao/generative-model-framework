...
class Seq2seq(nn.Module):
    def __init__(self, v):
        ...
        self.lambda_cov = torch.tensor(1., requires_grad=False, device=self.DEVICE)
    ...
    def forward(self, x, x_len, y, len_oovs, batch):
        ...
        for t in range(y.shape[1]-1):
            ...
            p_vocab, decoder_states = self.decoder(decoder_input_t.unsqueeze(1), decoder_states, encoder_output, context_vector)  # p_vocab (batch_size, vocab_size)
        ...
        sample_losses = torch.sum(torch.stack(step_losses, 1), 1)  # torch.stack(step_losses, 1) : (batch_size, y_len)     sample_losses : (batch_size)
...
#     @timer('final dist')
    def get_final_distribution(self, x, p_gen, p_vocab, attention_weights, max_oov):
        ...
        Returns:
            ...
            The shape is (batch_size, extended_vocab_size)
        """
        ...
        # Refer to equation (9).
        # p_vocab_extended[i][x[i][j]] += attention_weighted[i][j] # 如果 dim == 1
...
#     @timer('model forward')
    def forward(self, x, x_len, y, len_oovs, batch, num_batches, teacher_forcing):
        ...
        for t in range(y.shape[1]-1):
            ...
            # target_probs[i][j] = final_dist[i][y_t[i][j]]  (batch_size, 1)
            ...
            # 如果之前该词出现过了，那么它的coverage_vector就很大，那么为了减少loss，就需要attention_weights变小，小就代表着这个位置被注意的概率减少。
            if config.coverage:
                # Add coverage loss.
                ct_min = torch.min(attention_weights, coverage_vector)  # (batch_size, seq_len)
                cov_loss = torch.sum(ct_min, dim=1)  # (batch_size)
                loss = loss + config.LAMBDA * cov_loss
                coverage_vector = next_coverage_vector  # TODO
